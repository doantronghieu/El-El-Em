...
from langchain.chains import (ConversationalRetrievalChain, FlareChain,
                              OpenAIModerationChain, SimpleSequentialChain)
...
def configure_retriever(docs: list[Document], use_compression: bool = False
                        ) -> BaseRetriever:
    """
    Retriever to use.
    """

    # Split each document's documents
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=1500,
                                                   chunk_overlap=200)
    splits = text_splitter.split_documents(docs)

    # Create embeddings, store in vectordb
    embeddings = OpenAIEmbeddings()
    # Create vectordb with single call to embedding model for texts
    vectordb = DocArrayInMemorySearch.from_documents(documents=splits,
    ...
    return ContextualCompressionRetriever(base_compressor=embeddings_filter,
...
def configure_chain(retriever: BaseRetriever, use_flare: bool = True) -> Chain:
    ...
    Passing in a max_tokens_limit amout automatically
    Truncates the tokens when prompting llm.
    """
...
def configure_retrieval_chain(uploaded_files,
    ...
    """
    Read documents, configure retriever and chain
    """
